...
// Renvoie le fragment overpass pour chercher les types de lieux de tls, qui doivent tous correspondre au tag « catégorie »
...
export default async function chercheLieux(carte: L.Map, types_de_lieux: TypeLieu[]): Promise<LieuOsm[]> {
    ...
    const lieux = res_overpass.elements.map(
        (x: OverpassRes) => LieuOsm.fromOverpass(x, types_de_lieux, carte)
    );
